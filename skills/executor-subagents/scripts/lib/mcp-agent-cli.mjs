 ...
 * guessing from config-file conventions the way `checkContext7Mcp`/
 * `checkCodebaseMemoryMcp` in `preflight.mjs` do.
 *
 * Why this exists: those checks infer "is codebase-memory available to Codex"
 * by checking whether a marker string appears somewhere under
 * `~/.codex/config.toml`, and have no AGY-specific candidate path for
 * codebase-memory at all in `mcp-candidates.mjs` (only Context7 got
 * AGY-specific candidates). Both CLIs expose a real `mcp list` subcommand
 * that reports exactly what that specific CLI has registered and enabled —
 * confirmed against live output (2026-08-24, codex-cli 0.148.0, agy 1.1.17):
 *
 ...
 *
 * Redaction rule (same as the existing preflight checks): only `name`,
 * `enabled`/`status` and `transport.type` are ever extracted. Header, env,
 * URL and command-line content is discarded before it leaves the parser —
 * never forwarded, never logged.
 *
 * Pure in the same sense as the rest of this plugin's detection code: the
 * actual `child_process` call
