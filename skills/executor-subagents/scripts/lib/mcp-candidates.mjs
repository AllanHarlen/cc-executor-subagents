 ...
 * Context7 (both optional here — the Executor never treats either as
 * mandatory).
 *
 ...
 * `.kiro/settings/mcp.json` was "available" to Pensador but "absent" to the
 * Orchestrator's `mcp-detect.mjs`, and a server registered only in
 * `~/.codex/config.toml` was the reverse. `cc-pensador/test/mcp-detection-parity.test.js`
 * asserts the sibling copies of this list (this file, `cc-pensador/scripts/lib/mcp-candidates.mjs`,
 * and the specs in `cc-orchestrador-subagents/.../scripts/lib/mcp-detect.mjs`)
 * stay in sync when checked out side by side in this combined workspace.
 *
 ...
 * TOML: `preflight.mjs` matches it with a raw substring test, never a
 * structured parse — writing a TOML parser for a single candidate is not
 * worth the maintenance cost. Every `"json"` candidate goes through
 * `readMcpServerMaps()`/`findMcpServer()` in `preflight.mjs`, which excludes
 * servers listed in `disabledMcpjsonServers`.
 */
 ...
 * a real false positive caught in `cc-pensador`'s equivalent test suite.
 */
