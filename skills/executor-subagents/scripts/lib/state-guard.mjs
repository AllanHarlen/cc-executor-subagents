/**
 * Decides whether a tool call may hand-write the durable run state of cc-executor-subagents.
 *
 * The run state is event-sourced by `executor-state.mjs` (`state.json` snapshot + `events.jsonl`
 ...
 *     Reads (cat, grep, jq) pass. `executor-state.mjs` itself never needs an exemption: its command
...
export const RUN_ROOTS = Object.freeze(['.executor']);
export const PROTECTED_NAMES = Object.freeze(['state.json', 'events.jsonl', '.state.lock']);

const STATE_CLI = 'executor-state.mjs';
