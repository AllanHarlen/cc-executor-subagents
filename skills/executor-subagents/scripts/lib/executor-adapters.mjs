...
/** Executores suportados. `claude-code` roda dentro da propria sessao Claude
 * (sem probe externo real), mas o adaptador aceita o rotulo para o mesmo
 * shape de saida funcionar quando um papel da Project_Config aponta para ele. */
const SUPPORTED_EXECUTORS = new Set(["codex", "agy", "claude-code"]);
...
/**
 * Normaliza a saida bruta de um subagente (`codex`, `agy` ou `claude-code`)
 * em um probe estavel `{executorStatus, reasonCode, ...}`. `UNKNOWN` forca
 * `adapter.authoritative: false` — um probe nao-autoritativo nunca fecha uma
 * task como `DONE`/`FAILED` sozinho.
 */
export function adaptExecutorProbe(executor, rawInput, options = {}) {
  const normalizedExecutor = String(executor ?? "").toLowerCase();
  if (!SUPPORTED_EXECUTORS.has(normalizedExecutor)) {
...
/** Adapta um conjunto `{tasks: {<taskId>: raw}}` (ou `{<taskId>: raw}` direto). */
export function adaptProbeSet(input, options = {}) {
  ...
  for (const [taskId, raw] of Object.entries(source)) {
    ...
    tasks[String(taskId)] = adaptExecutorProbe(executor, raw, options);
  }
