...
// --- Caminho canonico do arquivo --------------------------------------------

test("canonical project config path is <root>/.executor/project-config.md", () => {
  const root = projectRoot();

  assert.equal(PROJECT_CONFIG_DIRECTORY, ".executor");
  assert.equal(PROJECT_CONFIG_FILENAME, "project-config.md");
  assert.equal(PROJECT_CONFIG_RELATIVE_PATH, ".executor/project-config.md");
  assert.equal(projectConfigPath(root), join(root, ".executor", "project-config.md"));
});
...
test("writeProjectConfig persists the file at the canonical path and reads it back", () => {
  ...
  assert.equal(written.path, join(root, ".executor", "project-config.md"));
...
// --- Tabela de opcoes e defaults por papel ----------------------------------
...
// --- Erros do parser em exemplos concretos ----------------------------------

test("parser error names the missing required field and the file path", () => {
  const error = captureError(() =>
    parseProjectConfig(fileWithout("frontendExecutor"), { path: "proj/.executor/project-config.md" }),
  );
  ...
  assert.equal(error.details.path, "proj/.executor/project-config.md");
  assert.match(error.message, /frontendExecutor/);
  assert.match(error.message, /proj\/\.executor\/project-config\.md/);
});
...
test("parser rejects content without any recognizable field line and unsupported schema versions", () => {
  const unparseable = captureError(() =>
    parseProjectConfig("# EXECUTOR PROJECT CONFIG\n\nnada aqui\n", { path: "proj/config.md" }),
  );
...
// --- Roteamento por tipo de trabalho (especifico do Executor) --------------

test("resolveExecutorForWorkType routes by work type and Project_Config, with FEATURE_SLICE split", async () => {
  const { resolveExecutorForWorkType } = await import(
    "../skills/executor-subagents/scripts/lib/project-config.mjs"
  );
  const config = { ...VALID_CONFIG, backendExecutor: "claude-code" };

  assert.deepEqual(resolveExecutorForWorkType("BUG", config), {
    tipo: "BUG",
    role: "backendExecutor",
    executor: "claude-code",
    executorSource: "project-config",
  });
  assert.deepEqual(resolveExecutorForWorkType("ui_frontend", config), {
    tipo: "UI_FRONTEND",
    role: "frontendExecutor",
    executor: "agy",
    executorSource: "project-config",
  });
  assert.deepEqual(resolveExecutorForWorkType("REVIEW", config), {
    tipo: "REVIEW",
    role: "backendReviewer",
    executor: "codex",
    executorSource: "project-config",
  });
  assert.deepEqual(resolveExecutorForWorkType("FEATURE_SLICE", config), {
    tipo: "FEATURE_SLICE",
    backend: "claude-code",
    frontend: "agy",
    executorSource: "project-config",
  ...
  assert.throws(
    () => resolveExecutorForWorkType("NOT_A_WORK_TYPE", config),
    (error) => error instanceof ProjectConfigError && error.code === "PROJECT_CONFIG_UNKNOWN_WORK_TYPE",
  );
});
